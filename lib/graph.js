...
function istKundenMail(absender) {
  ...
  return KUNDEN_DOMAINS.some(d => absender.toLowerCase().includes('@' + d.toLowerCase()) || absender.toLowerCase().endsWith(d.toLowerCase()));
}

// Holt neue Mails mit PDF-Anhängen seit timestamp
export async function fetchNeueBestellMails(seitTimestamp) {
  const client = getGraphClient();
  const seit = seitTimestamp ? new Date(seitTimestamp).toISOString() : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();

  // Mails mit Anhang seit Datum
